...
RcvMsgBuff *pRxBuff;
static char Recieved[RX_BUFF_SIZE];
char Tansmited[128];

extern void mParsData(char* Buf, u8 sender);
...
void RxUART0_task(os_event_t*e)
{
	if(e ->sig == SIG_RX)
	{
		int len = strlen(pRxBuff->pRcvMsgBuff);
		os_memcpy(Recieved,pRxBuff->pRcvMsgBuff,len);

		pRxBuff->pWritePos = pRxBuff->pRcvMsgBuff;
		Recieved[len]=0;
		 mParsData(Recieved,UART);
	}
}
